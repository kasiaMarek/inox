...
trait Expressions { self: Trees =>
  ...
  sealed case class Application(callee: Expr, args: Seq[Expr]) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = callee.getType match {
  ...
  sealed case class Forall(params: Seq[ValDef], body: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = body.getType
  }
  ...
  sealed case class CharLiteral(value: Char) extends Literal[Char] {
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = CharType()
  }
  ...
  sealed case class BVLiteral(signed: Boolean, value: BitSet, size: Int) extends Literal[BitSet] {
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = BVType(signed, size)
    def toBigInt: BigInt = {
  ...
  sealed case class IntegerLiteral(value: BigInt) extends Literal[BigInt] {
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = IntegerType()
  }
  ...
  sealed case class FractionLiteral(numerator: BigInt, denominator: BigInt) extends Literal[(BigInt, BigInt)] {
    ...
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = RealType()
  }
  ...
  sealed case class FPLiteral(exponent: Int, significand: Int, value: BitSet) extends Literal[BitSet] {
    override def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions) = FPType(exponent, significand)
  ...
  sealed case class BooleanLiteral(value: Boolean) extends Literal[Boolean] {
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = BooleanType()
  }

  /** $encodingof a string literal */
  sealed case class StringLiteral(value: String) extends Literal[String] {
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = StringType()
  }

  /** $encodingof the unit literal `()` */
  sealed case class UnitLiteral() extends Literal[Unit] {
    val value = ()
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = UnitType()
  }
  ...
  sealed case class GenericValue(tp: TypeParameter, id: Int) extends Expr with Terminal {
    def getType(using s: Symbols, options: TypeComputeOptions = TypeComputeOptions.NoOptions): Type = tp.getType
  }
  ...
  sealed case class IsConstructor(expr: Expr, id: Identifier) extends Expr with CachingTyped {
    override protected def computeType(using s: Symbols, options: TypeComputeOptions): Type = getADTType(expr) match {
  ...
  sealed case class ADTSelector(adt: Expr, selector: Identifier) extends Expr with CachingTyped {
    ...
    override protected def computeType(using s: Symbols, options: TypeComputeOptions): Type = getADTType(adt) match {
      case ADTType(id, tps) =>
        s.lookupSort(id)
  ...
  sealed case class LessThan(lhs: Expr, rhs: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = if (
  ...
  sealed case class BVNot(e: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getBVType(e)
  }

  /** $encodingof `... & ...` $noteBitvector */
  sealed case class BVAnd(lhs: Expr, rhs: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getBVType(lhs, rhs)
  }
  ...
  sealed case class BVNarrowingCast(expr: Expr, newType: BVType) extends Expr with CachingTyped {
    ...
    override protected def computeType(using Symbols, TypeComputeOptions): Type = cast match {
  ...
  sealed case class BVUnsignedToSigned(expr: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getBVType(expr) match {
  ...
  sealed case class FPUMinus(expr: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getFPType(expr)
  }
  ...
  sealed case class FPAbs(e: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getFPType(e)
  }

  sealed case class FPMax(lhs: Expr, rhs: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getFPType(lhs, rhs)
  }
  ...
  object RoundTowardZero extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = RoundingMode
  }
  ...
  sealed case class Tuple(exprs: Seq[Expr]) extends Expr with CachingTyped {
    ...
    override protected def computeType(using Symbols, TypeComputeOptions): Type = TupleType(exprs.map(_.getType)).getType
  }
  ...
  sealed case class TupleSelect(tuple: Expr, index: Int) extends Expr with CachingTyped {
    ...
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getTupleType(tuple) match {
  ...
  sealed case class SetAdd(set: Expr, elem: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getSetType(set) match {
  ...
  sealed case class SubsetOf(lhs: Expr, rhs: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getSetType(lhs, rhs) match {
  ...
  sealed case class SetIntersection(lhs: Expr, rhs: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getSetType(lhs, rhs)
  }
  ...
  sealed case class BagAdd(bag: Expr, elem: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getBagType(bag, BagType(elem.getType))
  }
  ...
  sealed case class MultiplicityInBag(element: Expr, bag: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getBagType(bag) match {
  ...
  sealed case class BagIntersection(lhs: Expr, rhs: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getBagType(lhs, rhs)
  }
  ...
  sealed case class FiniteMap(pairs: Seq[(Expr, Expr)], default: Expr, keyType: Type, valueType: Type)
    ...
    override protected def computeType(using Symbols, TypeComputeOptions): Type = checkParamTypes(
  ...
  sealed case class MapApply(map: Expr, key: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = getMapType(map) match {
  ...
  case class MapMerge(mask: Expr, map1: Expr, map2: Expr) extends Expr with CachingTyped {
    override protected def computeType(using Symbols, TypeComputeOptions): Type = (getMapType(map1, map2), getSetType(mask)) match {
